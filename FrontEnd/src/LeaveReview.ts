...
// The information sent to the backend
...
// The information returned from the backend
...
const submitButton =
  document.querySelector<HTMLButtonElement>(".btn-submit-review");

const skipButton =
  document.querySelector<HTMLButtonElement>(".btn-skip");

const textarea =
  document.querySelector<HTMLTextAreaElement>(".review-textarea");

const counter =
  document.querySelector<HTMLElement>(".char-counter");
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error(
          "Your login token is missing or expired."
        );
      }

      if (response.status === 403) {
        throw new Error(
          "Only the homeowner can submit this review."
        );
      }
      ...
      throw new Error(
        errorMessage || "Could not submit the review."
      );
    }
